...
def main():
    deck1 = [Card(1, "ギンヤンマ", 5, 1100, "赤", "n"), Card(2, "カブトムシ", 4, 600, "青", "SR")]
    deck2 = [Card(3, "ナミアゲハ", 4, 1100, "青", "R"), Card(4, "ナミアゲハ", 4, 1100, "青", "R")]

    player1 = HumanPlayer("Player 1", deck1)
    player2 = AIPlayer("AI", deck2)
    ...
    while not game_state.winner:
        ...
        if isinstance(current_player, HumanPlayer):
            game_ui.draw_game_state()
            selected_card_index = game_ui.selected_card_index
            if selected_card_index is not None:
                current_player.take_turn(game_state, selected_card_index)
    ...
    print(f"The winner is {game_state.winner.name}")
    game_ui.run()  # 勝利メッセージを表示し続ける

def check_winner(game_state):
    for player in game_state.players:
        if player.health <= 0:
            return game_state.players[1 - game_state.players.index(player)]
    return None
